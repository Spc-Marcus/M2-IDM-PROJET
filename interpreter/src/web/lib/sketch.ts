...
const sketch = (p: P5) => {
    p.setup = () => {
        const canvas = p.createCanvas(1000, 1000, document.getElementById("simulator"));
        ...
        win.lastTimestamp = 0;
        win.scene = null;
        win.p5robot = new Robot(1, p.width / 2, p.height / 2, 0, 0, 0, p);
    };
    ...
    p.draw = () => {
        ...
        if (win.scene !== null && win.scene.timestamps.length > win.lastTimestamp + 1) {
